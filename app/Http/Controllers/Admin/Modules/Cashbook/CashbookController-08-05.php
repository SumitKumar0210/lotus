    ...
    public function adminManageOpeningClosing(Request $request)
    {
        ...
        while ($from_date2 != $last_date2) {
            ...
            $expenses = Expense::where(
                'mode',
                'CASH'
            )->whereBetween('created_at', [$date_from, $date_to])->where('branch_id', '0')->get();
            if (!empty($expenses)) {
            ...
            if (!empty($branches)) {
                foreach ($branches as $branch) {
                    ...
                    //closing balance
                    $total_expenses = 0;
                    $expenses = Expense::where('mode', 'CASH')->whereBetween('created_at', [$date_from, $date_to])->where('branch_id', $branch->id)->get();
                    if (!empty($expenses)) {
                        foreach ($expenses as $item) {
