    ...
    <script>
        $(document).ready(function() {
            ...
            let table = $('#product_table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('branch-category.getCategoryList') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex'
                    },
            ...
            $(document).on('click', '.createProductButton', function(e) {
                ...
                if (category_name === '') {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ route('branch-category.store') }}",
                        data: formData,
                        cache: false,
                        contentType: false,
                        processData: false,
            ...
            $(document).on('click', '.editProduct', function() {
                ...
                //get show route
                let url = "{{ route('branch-category.show', ':update_id') }}";
            ...
            $(document).on('click', '.updateProductButton', function(e) {
                ...
                if (category_name_edit === '') {
                    ...
                    //get update route
                    let url = "{{ route('branch-category.update', ':update_id') }}";
                    url = url.replace(':update_id', update_id);
                    //get update route
                    ...
                    buttons: {
                        ok: {
                            action: function() {
                                ...
                                let url =
                                    "{{ route('branch-category.destroy', ':delete_id') }}";
