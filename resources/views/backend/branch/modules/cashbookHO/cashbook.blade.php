                    ...
                    <div class="table-responsive">
                        <table id="example2" class="table table-striped table-bordered w-100">
                            ...
                            <tbody>
                                ...
                                @if($item->opening_balance > 0 || $item->closing_balance > 0)
                                @if($item->branch_id == '0')
                                <tr>
                                    <td>{{$no++;}}</td>
                                    <td>
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <select name="branch" class="form-control select2 branch" id="branch">
                            ...
                            @if($item->id != 1)
                            @php
							$openings =\App\Models\OpeningBalance::where('branch_id',$item->id)->latest('opening_balance')->first();

                            @endphp
                            @if($openings->opening_balance <= 0)
								<option value="{{$item->id}}">{{$item->branch_name}}</option>
							@endif
							@endif
							@endforeach
...
<script>
    ...
    function getBranch()
    {
        ...
        let url = "{{route('branch.cashbookHO.cashbookBranch',':slip')}}";
                    ...
                    buttons: {
                        ok: {
                            action: function() {
                                ...
                                $.ajax({
                                    url: "{{route('branch.cashbookHO.updateOpeningBalance')}}",
                                    type: "POST",
    ...
    function manageOpeningClosing()
    {
        ...
        $.ajax({
                url: "{{route('branch.cashbookHO.adminManageOpeningClosing')}}",
                method:"POST",
