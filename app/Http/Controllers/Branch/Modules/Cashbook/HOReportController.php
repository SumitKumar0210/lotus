...
class HOReportController extends Controller
{
    // public function report()
    // {
    //     return view('backend.admin.modules.cashbook.report');
    // }
    ...
    public function getData(Request $request)
    {
        ...
        while ($date_from <= $date_to) {
            ...
            $opening_balance = 0;
            $last_closing_data = OpeningBalance::where('branch_id', $request->branch_id)->where('datetime', $nextdate)->get('closing_balance');
            ...
            //total balance
            $balance = OpeningBalance::where('branch_id', $request->branch_id)->where('datetime', $date_from)->get();
            $output .= '<tr>
            ...
            if (sizeof($payment_details) > 0) {
                foreach ($payment_details as $payment) {
                    $estimate_datas = Estimate::where('id', $payment->estimate_id)->where('branch_id', $request->branch_id)->get();
            ...
            if (sizeof($credit_notes) > 0) {
                foreach ($credit_notes as $note) {
                    $estimate_datas = Estimate::where('id', $note->estimate_id)->where('branch_id', $request->branch_id)->get();
                    $estimate_no = $estimate_datas[0]->estimate_no;
                    $customer = $estimate_datas[0]->client_name;
                    $mode = '';
                    $amount = 0;
