    ...
    <script>
        $(document).ready(function() {
            ...
            let table = $('#product_table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('warehouse-brand.getWarehouseBrandsList') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex'
                    },
            ...
            $(document).on('click', '.createProductButton', function(e) {
                ...
                if (brand_name === '') {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ route('warehouse-brand.store') }}",
                        data: formData,
                        cache: false,
                        contentType: false,
                        processData: false,
            ...
            $(document).on('click', '.editProduct', function() {
                ...
                //get show route
                let url = "{{ route('warehouse-brand.show', ':update_id') }}";
            ...
            $(document).on('click', '.updateProductButton', function(e) {
                ...
                if (brand_name_edit === '') {
                    ...
                    //get update route
                    let url = "{{ route('warehouse-brand.update', ':update_id') }}";
                    url = url.replace(':update_id', update_id);
                    //get update route
                    ...
                    buttons: {
                        ok: {
                            action: function() {
                                ...
                                let url =
                                    "{{ route('warehouse-brand.destroy', ':delete_id') }}";
